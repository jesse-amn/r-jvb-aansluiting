...
library(here)
library(tools)
factor_analyses <- read.csv(here(paste0("data/factor_analyses_simple", format(Sys.Date(), "%Y")), "/recommended_factors.csv"))

names <- list.files(here(paste0("data/factor_analyses_simple", format(Sys.Date(), "%Y"), "/")), full.names = TRUE)
...
# null hypothesis: empirical chi-square
p <- round(woordenrelateren$woordenrelateren_obli$EPVAL, 3) # < .05
chisq <- woordenrelateren$woordenrelateren_obli$chi

# model fit indices
rmsea <- woordenrelateren$woordenrelateren_obli$RMSEA[1] # < .06 or .08
tli <- woordenrelateren$woordenrelateren_obli$TLI # > 0.95
rms <- woordenrelateren$woordenrelateren_obli$crms # correct RMS, < 0.08

#  Explained variance
factor_r2 <- woordenrelateren$woordenrelateren_obli$R2

# Factor loadings
loadings <- woordenrelateren$woordenrelateren_obli$loadings # factor loadings
weights <- woordenrelateren$woordenrelateren_obli$weights # These are the factor score coefficients, which are used to compute factor scores for each observation in your data.
h2 <- woordenrelateren$woordenrelateren_obli$communality # communalities
u2 <- woordenrelateren$woordenrelateren_obli$uniqueness # uniquenesses

# Correlation matrix
corr_matrix <- woordenrelateren$woordenrelateren_obli$r
```
##### Model Beoordeling

Hieronder, in tabel 20, zie je de model-fit statistieken van de CFA voor woordenrelateren. Hierbij een korte uitleg wat het exact betekent:

- p-value: de p-waarde van de empirische chi-square. Als deze kleiner is dan .05, dan is er een significant verschil tussen de empirische en de geschatte chi-square. Dit betekent dat het model niet goed past.
- RMSEA: RMSEA meet hoe goed een model de waargenomen data benadert, rekening houdend met de fout in het model. Lagere waarden duiden op een betere pasvorm, typisch onder 0.05 voor goede pasvorm en tussen 0.05 en 0.08 voor redelijke pasvorm.
- TLI: de Tucker-Lewis Index. TLI is een pasvormindex gebruikt in structurele vergelijkingsmodellen om de relatieve verbetering van het gespecificeerde model te beoordelen in vergelijking met een basismodel. Waarden dicht bij 1 geven een betere pasvorm aan, met een gemeenschappelijke grenswaarde voor acceptabele pasvorm van 0.90 of hoger.
- CRMS: de Corrected Root Mean Square. Dit kwantificeert het gemiddelde verschil tussen waargenomen en voorspelde waarden in een model. Het geeft een maat voor hoe goed de voorwoordenrelaterenen van het model overeenkomen met de daadwerkelijke gegevens. Lagere RMRS-waarden geven een betere overeenkomst aan, wat wijst op een nauwkeuriger model.
...
Gezien de nummer van observaties (N =`r woordenrelateren$woordenrelateren_obli$n.obs`), is het verstandiger om de p-waarde van de empirische chi-square test als minder betrouwbaar te zien. Op basis hiervan,
als ook op basis van de recommended factors in figuur 1, wordt geadviseerd om het factor model niet te heroverwegen.
...
- ASL_woordenrelateren_017: 0.2906343
- ASL_woordenrelateren_004: 0.2663853
- ASL_woordenrelateren_006: 0.2520672
- ASL_woordenrelateren_016: 0.`r format(Sys.Date(), "%Y")`484
...
# null hypothesis: empirical chi-square
p <- round(metenenmeetkunde$metenenmeetkunde_obli$EPVAL, 3) # < .05
chisq <- metenenmeetkunde$metenenmeetkunde_obli$chi
metenenmeetkunde$metenenmeetkunde_obli$EPVAL
# model fit indices
rmsea <- metenenmeetkunde$metenenmeetkunde_obli$RMSEA[1] # < .06 or .08
tli <- metenenmeetkunde$metenenmeetkunde_obli$TLI # > 0.95
rms <- metenenmeetkunde$metenenmeetkunde_obli$crms # correct RMS, < 0.08

#  Explained variance
factor_r2 <- metenenmeetkunde$metenenmeetkunde_obli$R2

# Factor loadings
loadings <- metenenmeetkunde$metenenmeetkunde_obli$loadings # factor loadings
weights <- metenenmeetkunde$metenenmeetkunde_obli$weights # These are the factor score coefficients, which are used to compute factor scores for each observation in your data.
h2 <- metenenmeetkunde$metenenmeetkunde_obli$communality # communalities
u2 <- metenenmeetkunde$metenenmeetkunde_obli$uniqueness # uniquenesses

# Correlation matrix
corr_matrix <- metenenmeetkunde$metenenmeetkunde_obli$r
```
##### Model Beoordeling
...
De model-fit resultaten zijn positief. Ondanks een significante p-waarde van de empirische chi-square test (chi-square = `r chisq`, p < `r p`), zijn de andere model-fit indices goed. De RMSEA is goed, de TLI is acceptabel, en de CRMS is goed.
Gezien de gevoeligheid van p-waardes in empirische chi-square testen, en de hoge hoeveelheid observaties (N =`r metenenmeetkunde$metenenmeetkunde_obli$n.obs`), is het verstandiger
de andere fit indexen te hanteren. Tevens laat dit model een relatief goede explained variance zien van ongeveer 65 procent (R-squared: `r factor_r2`).
...
- ASL7_rekenen_MM_B_1F_023_pilot: 0.2965706
- ASL7_rekenen_MM_A_1F_014      : 0.2895972
- ASL7_rekenen_MM_A_1F_016      : 0.2741679
- ASL7_rekenen_MM_C_0F_008      : 0.2592118
- ASL7_rekenen_MM_A_0F_002      : 0.2511075
- ASL7_rekenen_MM_C_0F_010      : 0.2388022
- ASL7_rekenen_MM_B_0F_005      : 0.2330601
- ASL7_rekenen_MM_B_0F_007      : 0.1953028
...
Op basis hiervan, zijn er 2 cut-offs op basis van visuele inspectie van de grafiek. De eerste cut-off is een factor loading van .3, en de tweede cut-off is een factor loading van grofweg ~.2.
Er wordt geadviseerd op zijn minst de volgende vraag te heroverwegen.
...
names <- list.files(here(paste0("data/factor_analyses_simple", format(Sys.Date(), "%Y"), "/alternative_factors")), full.names = TRUE)
names <- names[grepl(".Rdata", names)]
